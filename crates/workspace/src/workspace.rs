...
impl Workspace {
        ...
        cx.subscribe_in(&project, window, move |this, _, event, window, cx| {
            match event {
                ...
                project::Event::DisconnectedFromHost => {
                    this.update_window_edited(window);
    ...
    fn handle_pane_event(
        ...
        match event {
            ...
            pane::Event::RemovedItem { item } => {
                ...
                self.update_window_edited(window);
    ...
    pub fn start_following(
        ...
        match leader_id {
            CollaboratorId::PeerId(_) => Some(cx.spawn_in(window, async move |this, cx| {
                this.update_in(cx, |this, window, cx| {
    ...
    fn update_window_edited(&mut self, window: &mut Window) {
    ...
    fn serialize_workspace_location(&self, cx: &App) -> Option<SerializedWorkspaceLocation> {
        if let Some(local_paths) = self.local_paths(cx) {
        ...
        cx.spawn_in(window, async move |workspace, cx| {
            ...
            workspace
                .update_in(cx, |workspace, window, cx| {
                    ...
                    workspace.update_window_edited(window);
                })
                .ok();
...
pub fn local_workspace_windows(cx: &App) -> Vec<WindowHandle<Workspace>> {
    cx.windows()
        ...
        .filter(|workspace| workspace.read(cx).is_ok())
        .collect()
}
